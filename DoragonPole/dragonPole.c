...
int calcCostPerformance(void)
{
  int i;

  for(i=0; i<ITEM; i++) {
      item[i].costPerformance = 
      (double)item[i].money / (double)item[i].weight;
  }
}

void sort(int *targetTable)
{
  int i = 0;
  int maxIndex = 0;
  
  if(maxIndex < item[i].costPerformance){
    maxIndex = i;
  }
